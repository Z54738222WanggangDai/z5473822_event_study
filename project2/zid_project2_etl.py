...
def read_prc_csv(tic, start, end, prc_col='Adj Close'):
    ...
    df.sort_index(inplace=True)  # Ensures the index is monotonic

        # Use slice to avoid errors on non-existing start or end dates
    df_time = df.loc[slice(start, end)]
    df_time = df_time.rename(columns={prc_col: tic.lower()}).dropna()

    ser = df_time[tic.lower()]
    return ser
...
def daily_return_cal(prc):
    ...
    """
    # <COMPLETE THIS PART>
    daily_returns = prc.pct_change()

    # Remove any NaN values that might have been generated, especially for the first element
    daily_returns.dropna(inplace=True)

    # Optionally, you can set the name of the series to be the same as the input series' name
...
def monthly_return_cal(prc):
    ...
    """
    # <COMPLETE THIS PART>
    monthly_count = prc.resample('ME').count()
    valid_months = monthly_count[monthly_count >= 18].index

    # Resample to get the last trading day's close of each month and convert to period index
    monthly_last = prc.resample('ME').last()
    monthly_last.index = monthly_last.index.to_period('M')

    # Calculate monthly returns
    monthly_returns = monthly_last.pct_change()

    # Filter the returns to include only the valid months
    monthly_returns = monthly_returns[monthly_returns.index.isin(valid_months.to_period('M'))]

    # Clean up the series by dropping NaN values
...
def aj_ret_dict(tickers, start, end):
    ...
    for ticker in tickers:
        # Retrieve the adjusted close prices using read_prc_csv
        prc_series = read_prc_csv(ticker, start, end)

        # Calculate daily returns
        daily_returns = daily_return_cal(prc_series)
        daily_returns_dict[ticker.lower()] = daily_returns

        # Calculate monthly returns
        monthly_returns = monthly_return_cal(prc_series)
        monthly_returns_dict[ticker.lower()] = monthly_returns

    # Convert dictionaries of series into DataFrames
...
# Example usage:
# Assuming the necessary functions and CSV files are properly setup
# result = aj_ret_dict(['AAPL', 'TSLA'], '2010-06-25', '2010-08-05')
# print(result)
...
if __name__ == "__main__":
    ...
    #ser_price = read_prc_csv(tic='AAPL', start='2020-09-03', end='2020-09-09')
    #print(daily_return_cal(ser_price))
    # _test_daily_return_cal(made_up_data=False, ser_prc=ser_price)
    #
    # # use made-up series to test daily_return_cal function
    ...
    # # use AAPL prc series to test daily_return_cal function
    #ser_price = read_prc_csv(tic='AAPL', start='2020-08-31', end='2021-01-10')
    #print(monthly_return_cal(ser_price))
    # _test_monthly_return_cal(made_up_data=False, ser_prc=ser_price)
